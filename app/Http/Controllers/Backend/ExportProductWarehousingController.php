...
class ExportProductWarehousingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->pw_customer_id) {
            $products = $products->where('pw_customer_id', $request->pw_customer_id);
        }
        ...
        return view('backend.product_warehousing.index', compact('products', 'listProducts', 'suppliers', 'customers', 'type'));
    }

    public function warranty(Request $request, $id)
    {
        $product = ProductWarehousing::with([
            'product' => function($product)
            {
                $product->select('*')->with('unit');
            },
        ...
        if (!$product) {
            return redirect()->route('get.list.export.product.goods_issue')->with('danger', '[Error] Đã xảy ra lỗi dữ liệu không tồn tại.');
        }

        return view('backend.product_warehousing.warranty', compact('product'));
    }
            ...
            return Excel::create('danh-sach-xuat-hang-'.Carbon::now(), function($excel) use ($products) {
                ...
                $excel->sheet('Danh sách sản phẩm đã xuất kho', function ($sheet) use ($products) {
                    ...
                    foreach ($products as $key => $product)
                    {
                        ...
                        $sheet->cell('C'.$i, $product->customer->count() > 0 ? $product->customer->ct_name : '');
