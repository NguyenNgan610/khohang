...
$(function () {
    ...
    $('#location').change(function () {
        ...
        $.ajax({
            ...
            if (result.code == 200) {
                var html = '<option value="">Chọn ô vị trí</option>';
                result.boxs.forEach(function (index, item) {
                    html += '<option value="'+index.id+'">'+ index.lb_name+'</option>'
                })
    ...
    $(document).on('change', '.pw_product_id', function () {
        var idPro = $(this).val();
        var location_id = $(this).parent().parent().attr('location');
        var url = $('#table-import-product').attr('urlGetListImei')
        ...
        $.ajax({
            ...
            if (result.code == 200) {
                var html = '';
                result.imeis.forEach(function (index, item) {
                    html += '<option value="'+index.pwd_imei+'">'+ index.pwd_imei+'</option>'
                })
                $('.list_imei_sp_'+ location_id).html(html);
            }
        });

        console.log([idPro, location_id, url])
    })
