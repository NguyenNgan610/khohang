<form role="form" action="" method="POST" enctype="multipart/form-data">
    ...
    <div class="box-body">
        ...
        <div class="form-group {{ $errors->has('p_status') ? 'has-error' : '' }}">
            ...
            <div class="checkbox" style="display: inline">
                <label>
                    <input type="radio" value="2" name="p_status" {{ old('p_status', isset($product) ?  $product->p_status : '') == 2 ? "checked='checked'" : ""  }}> Tạm dừng
                </label>
            </div>
        ...
        {{--<div class="form-group {{ $errors->has('p_location_id') ? 'has-error' : '' }}">--}}
            ...
            {{--<select name="p_location_id" class="form-control" id="location" url="{{ route('ajax.get.box') }}">--}}
                {{--<option value="">Chọn vị trí</option>--}}
                {{--@if($locations)--}}
                    {{--@foreach($locations as $location)--}}
                        {{--<option  {{old('p_location_id', isset($product) ? $product->p_location_id : '') == $location->id ? 'selected=selected' : '' }}  value="{{$location->id}}">{{$location->l_name}}</option>--}}
                    {{--@endforeach--}}
                {{--@endif--}}
            {{--</select>--}}
            {{--<span class="text-danger"><p class="mg-t-5">{{ $errors->first('p_location_id') }}</p></span>--}}
        {{--</div>--}}

        {{--<div class="form-group {{ $errors->has('p_location_id') ? 'has-error' : '' }}">--}}
            {{--<label for="exampleInputEmail1">Ô vị trí</label>--}}
            {{--<select name="p_box_id" class="form-control" id="location-box">--}}
                {{--<option value="">Chọn ô vị trí</option>--}}
                {{--@if($boxs)--}}
                    {{--@foreach($boxs as $box)--}}
                        {{--<option  {{old('p_box_id', isset($product) ? $product->p_box_id : '') == $box->id ? 'selected=selected' : '' }}  value="{{$box->id}}">{{$box->lb_name}}</option>--}}
                    {{--@endforeach--}}
                {{--@endif--}}
            {{--</select>--}}
            {{--<span class="text-danger"><p class="mg-t-5">{{ $errors->first('p_location_id') }}</p></span>--}}
        ...
        <div class="form-group {{ $errors->has('p_warranty_period') ? 'has-error' : '' }}">
            <label for="exampleInputEmail1">Thời gian bảo hành ( theo tháng )</label>
            <input type="number" class="form-control" name="p_warranty_period" value="{{ old('p_warranty_period', isset($product) ? $product->p_warranty_period : '') }}" placeholder="Vd : 1 tháng ">
            @if($errors->has('p_warranty_period'))
                <span class="help-block">{{$errors->first('p_warranty_period')}}</span>
            @endif
        </div>

        <div class="form-group {{ $errors->has('p_description') ? 'has-error' : '' }}">
            <label for="exampleInputEmail1">Ghi chú </label>
            <textarea name="p_description" id="" class="form-control" cols="30" rows="3" placeholder="Ghi chú ...">{{old('p_description', isset($product->description) ? $product->description : '')}}</textarea>
