<form role="form" action="" method="POST">
    ...
    <div class="box-body">
        ...
        <div class="form-group {{ $errors->has('w_note') ? 'has-error' : '' }}">
            <label for="exampleInputEmail1">Trạng thái nhập hàng <sup class="title-sup">(*)</sup></label>
            @php
    $importStatuses = [
        'Đang vận chuyển' => 'Đang vận chuyển',
        'Đang kiểm hàng' => 'Đang kiểm hàng',
...
<select name="w_note" class="form-control">
    @foreach($importStatuses as $key => $status)
                    ...
                    @foreach($warehousing->warehousingProduct as $keys => $productData)
                        <tr location="{{ $keys }}" class="product_{{ $keys }}">
                            ...
                            <td>
                                ...
                                <input type="number" class="form-control mg-t-10 pw_custom_price" oninput="if(value.length > 8 )value=value.slice(0, 15)" name="pw_custom_price[{{$keys}}]"
                                       value="{{ $productData->pw_custom_price }}" placeholder="Giá sản phẩm" style="display: {{ $productData->pw_active_price == 4 ? 'block' : 'none' }}">
                            </td>
                            <td style="width: 10%">
